...
export default function EPIs() {
  ...
  function getBadgeClass(status: string) {
    switch (status) {
      case "ativo": return "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800";
      case "vencido": return "bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-800";
      case "sem_estoque": return "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ShieldCheckIcon className={`h-8 w-8 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">
              Gestão de EPIs <span className="text-slate-400 dark:text-slate-500 font-medium text-lg">({epis.length})</span>
            </h1>
          </div>
          <p className="text-slate-500 dark:text-slate-400 font-medium ml-10 text-sm md:text-base">Controle de Certificados de Aprovação e estoque</p>
        </div>

        <button
          className="bg-blue-600 text-white flex items-center justify-center px-6 py-3 rounded-2xl font-bold shadow-lg shadow-blue-200 dark:shadow-none hover:bg-blue-700 transition-all active:scale-95 text-sm md:text-base"
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white pl-12 pr-4 py-3 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all shadow-sm"
            value={busca}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {pagina.map((e) => (
                <tr key={e._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800 font-bold">
                        {e.nome.charAt(0)}
                      </div>
                      <div>
                        <p className="font-bold text-slate-800 dark:text-slate-200 mb-0.5">{e.nome}</p>
                  ...
                  <td className="px-6 py-5">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md block w-fit border dark:border-slate-700">
                        {e.categoria.replace(/_/g, " ")}
                      </span>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 italic">{e.nivel_protecao}</p>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                      <ArchiveBoxIcon className={`h-4 w-4 ${e.estoque < 5 ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'}`} />
                      <span className={`text-sm font-black ${e.estoque < 5 ? 'text-amber-600 dark:text-amber-500' : 'text-slate-700 dark:text-slate-300'}`}>
                        {e.estoque} un
                      </span>
                    </div>
                  </td>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[75vh] md:max-h-[80vh]">
          <div className="flex-1 overflow-y-auto px-1 py-4 space-y-5 scrollbar-hide">
            ...
            <div className="space-y-1">
              ...
              <input type="text" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white p-3 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" placeholder="Ex: Protetor Auditivo Plug..." value={nome} onChange={(e) => setNome(e.target.value)} required />
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 ml-1">Categoria</label>
              <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white p-3 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 appearance-none cursor-pointer" value={categoria} onChange={(e) => setCategoria(e.target.value)} required>
                <option value="protecao_auditiva" className="dark:bg-slate-900">Proteção auditiva</option>
                <option value="protecao_visual" className="dark:bg-slate-900">Proteção visual</option>
                <option value="protecao_respiratoria" className="dark:bg-slate-900">Proteção respiratória</option>
                <option value="protecao_maos" className="dark:bg-slate-900">Proteção mãos</option>
                <option value="protecao_cabeca" className="dark:bg-slate-900">Proteção cabeça</option>
                <option value="protecao_pes" className="dark:bg-slate-900">Proteção pés</option>
                <option value="protecao_quedas" className="dark:bg-slate-900">Proteção quedas</option>
                <option value="protecao_corpo" className="dark:bg-slate-900">Proteção corpo</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="number" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white p-3 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" placeholder="00000" value={ca || ""} onChange={(e) => setCa(Number(e.target.value))} required />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 ml-1">Validade do CA</label>
                <input type="date" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white p-3 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" value={validadeCa} onChange={(e) => setValidadeCa(e.target.value)} required />
              </div>
            </div>
