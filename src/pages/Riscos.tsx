...
export default function Riscos() {
  ...
  function getBadgeClass(c: string) {
    switch (c) {
      case "baixo": return "bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800";
      case "moderado": return "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800";
      case "medio": return "bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800";
      case "alto": return "bg-rose-100 text-rose-700 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800";
      case "critico": return "bg-red-200 text-red-900 border-red-300 animate-pulse dark:bg-red-900/50 dark:text-red-200 dark:border-red-700";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ExclamationTriangleIcon className="h-8 w-8 text-rose-600 dark:text-rose-500" />
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Mapa de Riscos</h1>
          </div>
          <p className="text-slate-500 dark:text-slate-400 font-medium md:ml-10 text-sm md:text-base">Identificação e controle de perigos por setor</p>
        </div>

        <button
          className="w-full md:w-auto bg-rose-600 text-white flex items-center justify-center px-6 py-3 rounded-2xl font-bold shadow-lg shadow-rose-200 dark:shadow-none hover:bg-rose-700 transition-all active:scale-95 text-sm md:text-base"
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white pl-12 pr-4 py-3 rounded-2xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 outline-none transition-all shadow-sm placeholder:text-slate-400 dark:placeholder:text-slate-600"
            value={busca}
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Risco / Categoria</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Setor</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Classificação / Status</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Responsável</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 text-center">Sentinel AI</th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {pagina.map((r) => (
                <tr key={r._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group">
                  <td className="px-6 py-5">
                    <div>
                      <p className="font-bold text-slate-800 dark:text-slate-200 mb-0.5">{r.nome}</p>
                      <span className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500 tracking-wider">{r.categoria}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className="text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-lg whitespace-nowrap">
                      {r.setorId?.nome}
                    </span>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5 text-center">
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-4 py-2 rounded-xl text-xs font-bold hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-500 transition-all shadow-sm border border-indigo-100 dark:border-indigo-800"
                    >
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)} title={editingId ? "Ajustar Risco" : "Registrar Novo Risco"}>
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[75vh] md:max-h-[80vh]">
          <div className="flex-1 overflow-y-auto px-1 py-4 space-y-6 scrollbar-hide">
            ...
            <div className="bg-white dark:bg-slate-900/50 p-5 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
